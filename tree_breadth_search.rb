...
class Tree
  ...
  def breadth_first_search(target_key)
    ...
    current = queue.shift #removes first element from queue
    if current.key == target_key
      return current
    end
    queue = queue + current.children  #if you didn't find target key, queue now is previous queue plus node keys of current node's children


  end
  nil #If loop returns nil, it means all nodes were inspected and target key was not found
end
